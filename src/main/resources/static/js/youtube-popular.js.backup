...
let allVideos = []; // 전체 영상 데이터 저장
let currentShowRank = true; // 현재 랭킹 표시 여부
    ...
    try {
        ...
        if (data.success && data.videos) {
            if (data.videos.length === 0) {
                ...
                allVideos = data.videos; // 전체 데이터 저장
...
function generateTips(videos, avgDuration, avgEngagement, avgTitleLength) {
    const tips = [];
    ...
    if (durationMatch) {
        ...
        if (mins <= 5) {
            ...
            tips.push('긴 영상도 인기! <strong>깊이 있는 콘텐츠</strong>로 승부해보세요');
        }
    }
    
    // 제목 길이 팁
    if (avgTitleLength < 30) {
        tips.push(`제목은 <strong>${avgTitleLength}자</strong> 내외로 간결하게 작성하세요`);
    } else {
        tips.push(`제목에 <strong>키워드</strong>를 충분히 포함하되 핵심을 앞에 배치하세요`);
    }
    ...
    if (engRate >= 3) {
        ...
        tips.push('영상 끝에 <strong>좋아요/구독 요청</strong>과 <strong>질문</strong>을 넣어보세요');
    }
